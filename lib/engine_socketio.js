...
SocketIoEngine.prototype.loadContextSocket = function(namespace, context, cb) {
  ...
  if(!context.sockets[namespace]) {
    ...
    let path = this.path || {};
    let query = context.query || this.query || {
        query: {
          clusterId: context.vars.clusterId,
          session: decodeURIComponent(context.vars.session)
        }
    };
...
SocketIoEngine.prototype.compile = function (tasks, scenarioSpec, ee) {
  ...
  function zero(callback, context) {
    ...
    context._connectSocket = function(query) {
          options.query = query;
          context.query = query;
          const target = context.vars.target;// + '?' + options.query;
      ...
      ee.emit('started');
      return callback(null, _.extend({socketio: null}, context));
  }
